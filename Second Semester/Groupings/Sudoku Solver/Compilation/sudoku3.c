...
//Functions for the 3x3 sudoku box
// Stylization functions
void line_3(){printf("\n-----------------------------------------------------");}
void line1_3(){printf("-----------------------------------------------------");}
void verticalBorder_3(){printf("\n\t\t    \033[32m-------------\033[0m\n");}
void grid_3(int j){printf("%s", j == 0 ? "\t\t    \033[32m|\033[0m" : "");}
void grid1_3(int j){printf("%s", j < 3 ? "\033[32m|\033[0m" : "");}

// To print the 3*3 Sudoku Block
void printSudokuBlock_3(int sudokuBlock[3][3]){
    for (int i = 0; i < 3; i++) {
        verticalBorder_3();
        for (int j = 0; j < 3; j++) {
            grid_3(j);
...
void putInputToPositions(int sudokuBlock[3][3], int input[3], int posInput[3], int i) {
        ...
        printSudokuBlock_3(sudokuBlock);
}
